...
@section('title', 'Edit Event - Little Stars Daycare and Nursery School')
...
<div class="container-xxl py-5 page-header position-relative mb-5">
    <div class="container py-5">
        <h1 class="display-2 text-white animated slideInDown mb-4">Edit Event</h1>
        <nav aria-label="breadcrumb animated slideInDown">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('home') }}">Home</a></li>
                <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
                <li class="breadcrumb-item"><a href="{{ route('auth.events.index') }}">Events</a></li>
        ...
        <div class="row justify-content-center">
            <div class="col-lg-8">
                <div class="bg-light rounded p-5">
                    ...
                    <form action="{{ route('auth.events.update', $event) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            @if($event->hasFeaturedImage())
                                <div class="mb-2">
                                    <img src="{{ $event->featured_image_url }}" alt="Current featured image" class="img-thumbnail" style="max-width: 200px; max-height: 150px;">
                                    <p class="text-muted small mt-1">Current featured image</p>
                                </div>
                            @endif
                            <input type="file" class="form-control @error('featured_image') is-invalid @enderror" id="featured_image" name="featured_image" accept="image/*">
                            <small class="form-text text-muted">Upload a new featured image to replace the current one (JPEG, PNG, JPG, GIF, SVG - Max 2MB)</small>
                        ...
                        <div class="mb-4">
                            ...
                            <small class="form-text text-muted d-block mt-2">
                                <strong>Draft:</strong> Event is not visible to the public<br>
                                <strong>Published:</strong> Event is visible to the public<br>
                                <strong>Cancelled:</strong> Event is cancelled and not visible<br>
                                <strong>Completed:</strong> Event has been completed
                            </small>
                        </div>
                        
                        <div class="d-flex justify-content-between">
                            <a href="{{ route('auth.events.index') }}" class="btn btn-secondary">Cancel</a>
...
document.addEventListener('DOMContentLoaded', function() {
    // Enhanced multi-select for tags
    ...
    if (tagsSelect) {
        ...
        tagsSelect.addEventListener('change', function() {
            ...
            // Update the placeholder text to show selected count
            const label = this.previousElementSibling;
            if (selectedCount > 0) {
                label.innerHTML = `Tags (${selectedCount} selected)`;
            } else {
                label.innerHTML = 'Tags';
            }
        });
        
        // Trigger change event on page load to show initial selected count
